...
- `tcgplayer_orders` — real TCGPlayer order records, populated by a Gmail-based order sync (TCGPlayer's API program is closed to new access, so orders are parsed from sale-notification emails instead): `order_number, order_total, ship_by, status ('new'|'shipped'), gmail_message_id, manage_order_url, ordered_at, shipped_at`. Only `status`/`shipped_at` are ever written from this repo — via a manual "Mark shipped"/"Mark not shipped" toggle in `Tcgplayerlistings.jsx`'s Orders tab (`toggleOrderStatus`), for when the Gmail sync misses or lags a real shipment. Every other column is sync-owned. This `status` is a plain two-state ship-tracking flag, distinct from `tcgplayer_listings.status` (`active|sold|ended|cancelled`) — an order can be `'new'` (not yet shipped) against a listing that's already `sold` in this app, since marking a listing sold here just records the sale, not the physical shipment. No `game_id` — see `v_tcgplayer_orders` below.
- `tcgplayer_order_items` — line items per order: `order_id, card_id, card_name_raw, condition, foil, quantity`. `card_id` is only populated when `card_name_raw` was successfully matched against `cards.name` (including a literal " (Foil)" suffix for foil printings — do not strip it before matching); unmatched items keep `card_id` null and are only ever shown via `card_name_raw`, never used to drive a listing-row badge (see `v_tcgplayer_orders`/`card_ids` below).
...
- `pack_cards` — cards recorded within each pack: `pack_id, card_id, quantity, counts_inventory`. **`cards.quantity_owned` is not a plain column the app manages** — a `trg_sync_quantity_owned` trigger recomputes it after every insert/update/delete on `pack_cards` as `SUM(quantity) WHERE card_id = ... AND counts_inventory`, once any `pack_cards` row exists for that card. `Boxes.jsx`'s pull-logging flow (`savePull`) therefore only ever writes `pack_cards`, never `cards.quantity_owned` directly — its "Don't update inventory" checkbox works by setting `counts_inventory: false` on the row rather than skipping a manual update, since the pre-2026-09-04 version of that checkbox skipped only the JS-side update while the trigger added the pull's quantity back in anyway (see `supabase/migrations/20260904_pack_cards_counts_inventory.sql`). A card already logged in a pack is merged additively (existing quantity + new) when re-added via the "Add card" search rather than upserted-and-overwritten, for the same reason. Cards manually created/edited outside of pack pulls (Import.jsx, Inventory.jsx) are unaffected by the trigger until/unless a `pack_cards` row for them is created.
...
- `v_tcgplayer_orders` — one row per `tcgplayer_orders` row, with `tcgplayer_order_items` aggregated into `all_item_names` (raw names, ` ×N` suffixed like `all_card_names` elsewhere, ` (Foil)` suffixed per item), `matched_card_ids` (distinct non-null `card_id`s across the order's items), `total_quantity`, and `unmatched_item_count`. Not game-scoped (no `game_id` column) — like `v_global_pnl`/`v_tcgplayer_pnl`, `Tcgplayerlistings.jsx`'s Orders tab shows every order regardless of the active game, since `tcgplayer_orders` has no game linkage of its own and an order can span cards from either game. `v_tcgplayer_active`/`v_tcgplayer_sold` each also expose a `card_ids` array (the listing's `card_id` if single-card, else the distinct `tcgplayer_listing_cards.card_id`s for a lot) so listing rows can be matched against `v_tcgplayer_orders.matched_card_ids` — see `ordersForCardIds()` in `Tcgplayerlistings.jsx`. This matching is card_id-only by design: an order line with an unmatched `card_name_raw` never drives a listing-row badge, only the raw text shown in the Orders tab, since a name-based fallback risked pointing at the wrong card. Because a single card can be sold more than once, a badge match doesn't guarantee it's *that specific* sale's order — it's informational ("some order touching this card is pending/shipped"), not an exact 1:1 reconciliation. See `supabase/migrations/20260915_tcgplayer_orders_listing_link.sql`.
...
A third tab, **Orders**, reads `v_tcgplayer_orders` (real orders synced from Gmail into `tcgplayer_orders`/`tcgplayer_order_items` — see the tables section above). Order data itself (items, totals, dates) is sync-owned and not editable here, but each row has a manual "Mark shipped"/"Mark not shipped" button (`toggleOrderStatus`) that writes `status`/`shipped_at` straight to `tcgplayer_orders`, for when the Gmail sync misses or lags a real shipment. It's the only place in this repo that surfaces real TCGPlayer shipping status (Ready to ship / Overdue / Shipped, `ship_by` date, a link to `manage_order_url`), distinct from a listing's own `active|sold` status. The Active and Sold tabs additionally show a small badge on any row whose card(s) match a real order's `matched_card_ids` (`📦 Order pending`/`Order overdue` on Active, `📦 Awaiting shipment`/`✓ Shipped` on Sold) — see `ordersForCardIds()` and the `v_tcgplayer_orders` view note above for how that matching works and its limits. Closing out an active listing via the Sold modal (first-time save, not editing an already-sold row) reuses the same `ordersForCardIds()` match against the listing's (possibly just-edited) linked cards and auto-marks any still-`'new'` matching order `shipped` (`shipped_at` = now) — the assumption being that if you're recording the card as sold here, it physically shipped too. This only ever promotes `new` → `shipped`; it never touches an order that's already `shipped`, and it's still just a best-effort card_id match (same caveats as the badges), not a guaranteed link to *that* specific sale's order.
